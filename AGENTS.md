...
- **命名规范**：
  ...
  - **严禁自创非通用缩写与随意截断拼写**：
    - 业务领域概念、功能模块、文件与目录名、路由路径、组件、函数、变量、类型与 Query Key 必须使用完整、确切的标准英文单词，严禁自行随意缩写或掐头去尾截断词根。
    - 仅允许业界公认且无二义性的通用首字母缩写以及前端生态既定内建约定。
    - 领域术语全链路强对齐：同一业务实体在路由、菜单配置、文案命名空间、目录路径、Service、类型定义及状态管理中必须保持完全一致的英文拼写，严禁同一概念在不同层级中全称与缩写混用或各自为政。
  - 仅约束新建路径或明确重构，不批量重命名已有存量文件。

## 国际化文案

- `src/locales/` 是语言域的唯一外部入口。运行时装配在 `src/runtime/bootstrapRuntime.ts` 中 `await setupI18n()`，卸载时必须调用 `cleanupI18nEffects()` 停止监听宿主语言事件，否则微应用卸载后仍会响应宿主广播。
- 文案按 `public/locales/<lang>/<ns>.json` 存放，命名空间声明清单是 `src/locales/config.ts` 的 `I18N_NAMESPACES`，默认命名空间为 `home`。声明清单、各语言目录下的文件与键集三者必须对齐，漂移不会导致构建失败。
- 语言以宿主为准：初始语言由宿主传入，未传入时由 `resolveInitialLocale()` 解析，其后通过语言副作用跟随宿主事件。微应用不自行决定语言，也不向宿主广播。
- 脚手架只自带微应用自身与语言命名空间，不预置业务领域命名空间；业务领域的命名空间划分与文案粒度使用对应的国际化文案工作流。
